...
def main(args):
    ...
    def count_invalid_outputs(example):
        ...
        for k in ks:
            ...
            if not top_k_predictions:
                ...
                invalid_output_count = sum(1 for item in top_k_predictions if item not in permuted_candidates)
    ...
    print(f"Before Lora Model size: {model_size_gb:.2f} GB")
    ...
    if lora:
        result_path = os.path.join(result_path, 'instruct')
    ...
    dtype_size = torch.finfo(model.dtype).bits / 8  
    model_size_gb = (num_params * dtype_size) / (1024**3)  
    print(f"After Lora Model size: {model_size_gb:.2f} GB")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lora', type=bool, help="Whether lora weights are added", default= True)
    parser.add_argument('--lora_weights', type=str, help="The path to the weights", default= "llm_models/ml-1m/ml_k_all_c_all")
